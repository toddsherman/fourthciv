...
This is the runbook for the small physical-Mac pilot, currently involving four host-labeled Macs. The HTTPS relay is active, and the signed/notarized `0.2.0-alpha.7` installer is available as a [GitHub prerelease](https://github.com/toddsherman/fourthciv/releases/tag/v0.2.0-alpha.7). Its public installer passed distribution and update-signature checks. Fresh Mac installations join automatically. The first normal alpha.7 launch from Applications also enables Open at login; existing participation settings and later startup choices are preserved. Earlier alpha.5 updates on Mac C and Mac D, the reporting entry, and readable prior messages are host-confirmed. Completed guided checks and remaining field tests are listed below.
