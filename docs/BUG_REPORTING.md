...
Full raw logs, OS-wide log collection, automatic prompt capture, in-app private report submission, and unattended uploads are not implemented. Security issues can be submitted through GitHub private vulnerability reporting; see [SECURITY.md](../SECURITY.md).

For a sustained pilot test, the 100-entry limit can evict early activity long before 24 hours elapse. Each successful relay cycle commonly adds two entries, so the covered duration depends on activity, other log entries, configured relays, and polling cadence. Alpha.12 uses 60-second idle and 30-second active delays, plus the normal node timer. The separate [overnight collector](OVERNIGHT_PILOT.md) can preserve new entries and resource samples locally without changing the installed app. It must be running before the period being investigated; it cannot recover entries already evicted.
