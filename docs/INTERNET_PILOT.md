...
Status: [alpha.13/build 14](https://github.com/toddsherman/fourthciv/releases/tag/v0.2.0-alpha.13) is the current pilot installer, with guided drag/open steps. The request-isolated HTTPS relay is live at `https://fourthciv-pilot.vercel.app`. The September 12 rollout preserved all 39 signed events and the existing epoch. Preview platform-header checks, actual PostgreSQL concurrency, production migration, and release checks are recorded in [validation](VALIDATION.md). Guided separate-network, sleep/wake, reconnection, and overnight three-Mac checks remain documented separately. See [the host guide](PILOT_HOST_GUIDE.md).
...
Relay progress and usage are persisted in `internet-sync.json`. Usage resets at midnight UTC. Starting with alpha.12, a successful pass schedules its next poll 60 seconds later when no new events were received or shared and no work remains; active or unfinished exchanges retain a 30-second delay. Failures retry after 30 seconds, doubling up to five minutes. These are earliest deadlines; the normal node timer may run later. Published alpha.10 and alpha.11 use a 30-second successful-poll delay. Each pass receives up to 16 pages and shares up to 32 events per relay. All accepted events retain their original authors and signatures.
...
4. Run `npm ci --ignore-scripts`, then `npm run migrate` inside `relay/`. Migration creates the schema transactionally and inserts no conversations. Exportable production values may be pulled into the ignored `.env.production.local` using `--environment=production`, then used with `node --env-file=.env.production.local scripts/migrate.mjs`. Sensitive Vercel values are returned as `[SENSITIVE]` placeholders and cannot be used locally. Preserve that protection; use the existing database operator console or the reviewed production build-migration path in the [rollout guide](RELAY_HARDENING_ROLLOUT.md). Restrict any local credential files to mode 0600 and remove the production file afterward.
