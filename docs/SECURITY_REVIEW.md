...
**September 12 deployment record:** the reviewed request-isolation mitigation was deployed as `dpl_DHpwN8LJXrFnfYYzV8WfkpgDYeEW`, from source `78aa0cfd7fd2e5c08d9a842b5f31cfbbe57e50cf`. Protected preview tests confirmed platform header replacement, signed interoperability, seeded throttling/recovery, and quota races across four actual PostgreSQL sessions. The production migration preserved existing history, epoch, and old counters, with a restricted backup retained; public health, discovery, and all 39 signed events were verified after promotion. Alpha.12/build 13 separately ships the native changes. No production test posts, live flooding, or history reset were performed. GitHub private vulnerability reporting is enabled. See [the rollout record](RELAY_HARDENING_ROLLOUT.md) and [validation](VALIDATION.md); the residual risks below still apply.

**September 24 deployment record:** the CDN cache and bounded origin-work changes were promoted at approximately `2026-09-24T14:58Z` as `dpl_HEQxqN5fGFqCoRvXXvuH2yqjMLr2`, from verified source `ea28ba632816b5f6c6650f122213e6c5161f0a48`. Protected-preview checks and all three jobs in [CI run 36015810271](https://github.com/toddsherman/fourthciv/actions/runs/36015810271) passed, including native tests, relay interop, and outage recovery. [PR 1](https://github.com/toddsherman/fourthciv/pull/1) merged as `6799b0afd2bb48214e06122a0815a21895ed9a48`.

Production was already returning 503 before promotion. A private, never-promoted build used the existing production credential for read-only `SELECT 1`; it failed with `NeonDbError` and the sanitized quota flag, without an authentication flag. This identifies a quota-related failure, not the exact compute balance or reset date. The reviewed gate exception deployed the exact tested code before quota recovery to avoid resuming the old continuous SQL polling. The same 503 remained afterward; an uncached invalid-query 400 confirmed the new handler only. Full production history, successful caching, and installed-Mac CDN checks are **deferred, not passed**. The 05:11 UTC backup retains all 39 signed events, and the September 12 deployment remains available for rollback. No production post/replay, schema, data, environment-variable, or billing-plan change was made. The relay alone was excluded from paid Observability Plus; team settings and other projects were unchanged. See [the gate-exception record](RELAY_HARDENING_ROLLOUT.md#september-24-cache-rollout-during-a-quota-outage).
...
**Severity:** high for relay availability; no demonstrated code execution or credential exposure. **Baseline:** confirmed. **Mitigation:** implemented, locally tested, and deployed after the verified preview/production rollout. **Residual availability risks:** distributed abuse, shared-source allowances, and infrastructure cost remain as described below.
...
The baseline also had a normal-use capacity problem. Before alpha.12, each successful relay pass requested discovery and an events page (`Sources/FourthCivCore/Node.swift`, `syncRelays`) and scheduled the next pass 30 seconds later. With the default 10-second host loop and quick responses, approximately 30–40-second polling intervals meant 4,320–5,760 requests per continuously awake Mac per day. Four to five Macs could approach or exceed the old shared daily allowance before posts or other clients were included. This is an estimate from the baseline scheduler, not a live load measurement.

The deployed mitigation implements separate transport-derived source allowances and a larger shared admission allowance (`relay/lib/client.mjs`, `relay/schema.sql`, `fc_permit_request`). A source receives 120 admitted requests per minute and 60,000 per UTC day; the shared allowance is 1,200 per minute and 120,000 per UTC day. Eight Macs at the approximately 5,760-request upper estimate consume 46,080 per day, leaving room for one source to spend its 60,000-request allowance. This is a pilot sizing assumption, not a service guarantee.
...
This is a mitigation for a small pilot. Multiple independent sources can still exhaust a shared allowance, and rejected requests still consume function/database work and contend for the admission lock. Admitted-request quotas are not a hard cap on infrastructure cost or a complete denial-of-service defense. Vercel's documented request-header handling supports using the platform's `x-vercel-forwarded-for` at that adapter boundary; the alpha.12 preview verified actual header replacement before promotion. Repeat that platform check when changing the adapter or deployment environment. See [Vercel request headers](https://vercel.com/docs/headers/request-headers#x-vercel-forwarded-for).

The September 24 implementation changes the scope of these counters when Vercel CDN caching is enabled: successful public GET hits bypass the function and database. Discovery declares `limits.requestScope: origin`; the standalone uncached handler declares `all`. POSTs and cache misses still use durable database admission. Invalid routes/queries and invalid publication envelopes/signatures are rejected before SQL, but still consume origin resources. Platform protection must account for total traffic; the PostgreSQL counters do not count CDN hits or all rejected input.

Only successful public reads receive a one-hour tagged CDN lifetime; clients retain `no-store`. POST responses, errors, and source-specific admission data are not cached. A preview race demonstrated that a pre-commit GET could refill stale content after an immediate tag deletion. The implementation therefore limits cacheable GET origin work to four seconds, checks elapsed time before returning a cacheable response, and waits five seconds after a publication commits before deleting its tag. Duplicate submissions use the same delay. The extra second is an ingress allowance, not a bound on provider buffering; this is a timing mitigation rather than strict consistency. Protected-preview checks passed for delayed reads below and above the GET deadline and concurrent publications; that evidence does not establish production behavior during the database outage.

After the delay, purge retries are bounded to three attempts within three seconds; the complete POST shares a twelve-second origin deadline. Delay or purge failure returns a redacted 503 even though the event may already be committed. Retrying its original signed ID is idempotent. A subsequent read may acknowledge that event without another POST, so retries alone do not guarantee repair of all cached copies. Degraded visibility relies on the one-hour cache expiry, then a successful origin refresh and client poll; an ongoing database outage can extend the delay.

Cached health is a timestamped history snapshot, not evidence that writes currently work. Regional misses, evictions, query variation, deployments, and new publications can still keep compute active. Caching is intended to reduce idle work, but production savings are unmeasured and it provides no hard hosting-cost cap. Future rollout checks must verify cache keys, actual cache hits, deletion and concurrent refill behavior, cancellation, and environment separation; local handler tests do not establish those platform properties. See [the cache interface notes](INTERNET_PILOT.md#cached-public-reads) and [the free-pilot plan](FREE_PILOT_PLAN.md).
...
1. Identify the failure using the app's reviewed diagnostic report, relay HTTP status/Retry-After, and operator-side capacity counters. When CDN caching is enabled, inspect the snapshot timestamp rather than treating a cached health 200 as proof of live write availability. Use provider control-plane metrics to observe idle compute without waking it with SQL probes. Keep credentials, raw addresses, and private host files out of public reports.
...
Remaining work includes stronger abuse/admission design, physical-Mac resilience checks, and an independent security review before making stronger security claims. The alpha.12 rollout completed its deployed-header, quota-concurrency, migration, and public-read verification; repeat these checks for future infrastructure changes.

The final alpha.12 source/publication preparation scan found zero suspected secrets and zero scan errors across 143 current text files and 503 reachable historical text blobs. Three current binaries, seven historical binaries, and one oversized historical blob were excluded. This remains a pattern scan, not proof that every possible secret is absent.
