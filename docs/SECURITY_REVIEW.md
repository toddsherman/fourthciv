...
The September 23 candidate changes the scope of these counters when Vercel CDN caching is enabled: successful public GET hits bypass the function and database. Discovery declares `limits.requestScope: origin`; the standalone uncached handler declares `all`. POSTs and cache misses still use durable database admission. Invalid routes/queries and invalid publication envelopes/signatures are rejected before SQL, but still consume origin resources. Platform protection must account for total traffic; the PostgreSQL counters do not count CDN hits or all rejected input.

Only successful public reads receive a one-hour tagged CDN lifetime; clients retain `no-store`. POST responses, errors, and source-specific admission data are not cached. A preview race demonstrated that a pre-commit GET could refill stale content after an immediate tag deletion. The candidate therefore limits cacheable GET origin work to four seconds, checks elapsed time before returning a cacheable response, and waits five seconds after a publication commits before deleting its tag. Duplicate submissions use the same delay. The extra second is an ingress allowance, not a bound on provider buffering; this is a timing mitigation rather than strict consistency. Delayed reads below and above the GET deadline and concurrent publications require platform verification before promotion.
...
Cached health is a timestamped history snapshot, not evidence that writes currently work. Regional misses, evictions, query variation, deployments, and new publications can still keep compute active. Caching reduces expected idle work but provides no hard hosting-cost cap. Preview checks must verify cache keys, actual cache hits, deletion and concurrent refill behavior, cancellation, and environment separation; local handler tests do not establish those platform properties. See [the cache interface notes](INTERNET_PILOT.md#cached-public-reads-source-candidate) and [the free-pilot plan](FREE_PILOT_PLAN.md).
