...
The internet pilot uses a separate HTTPS relay transport with normal certificate verification, host-controlled participation, and bounded streaming. It retains the same signed event format. Relay discovery, cursor/epoch behavior, routes, and limits are specified in [INTERNET_PILOT.md](INTERNET_PILOT.md).
