...
The alpha.8 release was packaged from clean source commit `17e3e35984d21324b3b204b7e1559bdfac97054d`; GitHub Checks run `34144005316` passed all three jobs. Apple accepted the app submission `2e01cd9c-f0a7-4086-acd7-a5906af26a33` and DMG submission `06c9af83-6180-421e-9f64-e79c4e2c6bd2` with no issues. Both architectures, strict signatures, stapling, distribution assessment, and bundled-CLI notarization checks passed. An isolated launch of the exact packaged binary reported alpha.8/build 9, retained saved pause and custom limits, and left the login-item preference unchanged. The exact public GitHub installer passed SHA-256 and Ed25519 checks before staging the feed, with all six previous entries retained. DMG SHA-256: `a92149520aec6525f212fc723affcfcfe6af1ac4c814f574e20953335110b8fd`. Evidence: `.local/release-check-alpha8` and `.local/notarization/0.2.0-alpha.8-{app,dmg}.json`. No normal installed app was replaced or public message posted during these checks.
...
## Pilot report export and antivirus alert — September 6, 2026 PDT

Following the Mac D report-export instructions, the host supplied the saved Markdown and reported: “I was able to save it locally, but my antivirus flagged it for trying to access protected files.” The supplied report contains valid diagnostic JSON with version `0.2.0-alpha.5`, build `6`, release configuration, arm64, and macOS `26.6.0`, captured at `2026-09-07T03:57:31Z`. It reports 23 stored events, saved diagnostic history, 32 activity entries with no recorded failures, and zero pending relay events. These are snapshot values, not an independent comparison of the remote store or proof of delivery. The alert occurred during the export flow; its exact timing has not been established, and it is not captured in this report.

Local saving and version/build metadata are confirmed by the host's report and supplied artifact. The host identified Bitdefender and tentatively recalled the Applications folder. The exact alert wording, named app and target file/folder, and whether the alert appeared when opening the Save dialog or after choosing Save remain unknown. No protection setting was changed by the agent or recommended during investigation, and no false-positive conclusion was made.

Source inspection of the released code found that report preparation uses app metadata and the node's diagnostic snapshot, while export opens `NSSavePanel` and writes the reviewed Markdown to the selected URL with Foundation's atomic write option. No personal-folder scan is implemented in this reporting flow. This inspection does not identify the operation that triggered the alert on the remote Mac. No application code was changed. Local evidence: `.local/alpha5-report-export-pilot/verification.json`.

Bitdefender's [Safe Files documentation](https://www.bitdefender.com/consumer/support/answer/2443/) describes protected-folder write restrictions, including Desktop, Documents, Downloads, and Pictures by default. Its [blocked-app troubleshooting guide](https://www.bitdefender.com/consumer/support/answer/1448/) separately identifies an application's own `/Applications` path. Therefore, the recalled Applications reference could identify the app rather than the protected destination; this is a hypothesis pending the actual alert. Do not treat a Safe Files access alert as a confirmed malware detection or a confirmed false positive without that evidence.

The host then reported selecting “trust application” or a similar action, with Fourth Civ now shown as **Allowed** in Bitdefender's **Application Access** list; the original alert could no longer be found. This is consistent with Bitdefender's documented Safe Files app-permission flow. Record the report export as successful with a host-reported app-specific trust action, and retain the initial prompt as a compatibility observation. The exact triggering operation remains unverified, and no application fix is claimed. Recreating the alert or changing protection settings is not needed to continue the pilot. If it recurs, capture the full alert before dismissing it; behavior across later restarts or updates has not been tested.

## Overnight collection preparation — September 6, 2026 PDT

After the host reported updating this Mac, direct inspection confirmed the installed app and running node are alpha.5/build 6. At `2026-09-07T04:27:09Z`, diagnostics reported 23 stored events, Internet pilot enabled, LAN disabled, and zero pending relay events. This is a current local snapshot, not an independent remote-receiver comparison.

Added `scripts/pilot_collect.py` and its runbook. The utility only invokes local `diagnostics` and signature-verifying `events`, plus process metrics for the exact installed-app executable. It deduplicates diagnostic activity into a private bounded sample file, records sampling gaps without assuming a cause, and retains only the expected sender/community/run's test-message IDs, sequence numbers, occurrence counts, and timing. It has no posting or identity-reading path and does not prevent receiver sleep. A twelve-hour run has a 12 MiB sample cap, explicit stop control, exclusive collector locking, and refuses to overwrite a previous run. It continues after its launching terminal or Codex task closes but does not survive reboot or process termination.
...
## Automatic participation release — September 7, 2026 PDT

At the host's request, fresh Mac installations now connect to the public pilot without a separate join step. Existing settings remain unchanged, including off/paused choices, relays, and custom limits. Initial settings are saved before network work. A pre-existing data directory without settings is treated as an older store and remains local; headless CLI and demo defaults remain local-only.

Released `0.2.0-alpha.6` (build 7) from clean commit `b25292051451bdd57b6058bf61aa0ae60d16494e`. The 27 Swift tests, real loopback integration suite, six collector tests, six release/update tests, and native app and website builds passed locally. All three GitHub Checks jobs passed for that source commit ([run 34138726749](https://github.com/toddsherman/fourthciv/actions/runs/34138726749)), including relay/PostgreSQL and Swift/relay interoperability. Four new first-run tests cover actual mocked receipt, default budgets, persisted choices, missing/legacy settings, empty directories, and headless defaults.

Both architectures were built in a clean detached checkout. Apple accepted app submission `2db552fa-fbc9-4d44-bd77-b2cee5ebd3da` and DMG submission `a6a39e06-23fa-48ef-836e-529c30e5beb1` without issues. The app and DMG are Developer ID signed and stapled. The final DMG was mounted read-only; its app and bundled CLI passed strict signature, distribution/notarization, architecture, and version checks.

The app copied from that final DMG started with a genuinely fresh isolated profile and received all 23 existing live-relay events automatically. The bundled release CLI verified every event's signature, and no duplicate IDs were present. A restart retained history and enabled participation. After stopping the isolated app and saving off/paused choices and lower limits in its test profile, another restart retained those settings and history; no relay attempt occurred during the paused observation. No new public message was created, and the normal installed app and pilot data were not replaced. Evidence: `.local/release-check-alpha6/verification.json` and `fresh-diagnostics.json`. This verifies a release app on the build Mac, not a fresh installation on another physical Mac.

The local install page was checked visually and through browser interaction: three steps, automatic-connection explanation, alpha.6 download, and no extra “Have an agent of your own?” paragraph. The footer still links to the agent connection guide. The install and connection pages reported no browser warnings/errors. Publication follows with the matching installer so the website does not describe behavior absent from its download.

Published the immutable DMG, checksum, and source manifest as the GitHub prerelease. Installer SHA-256: `2bf472e961ce6583e445779ee9058509cc70fa69e79f64467ea8095b581a6d76`. The publication script downloaded the actual public installer, verified its SHA-256 and Ed25519 signature and the signed feed, and preserved builds 6, 5, 4, and 3 before staging build 7. The following website publication carries that exact verified feed.


The website publication at `f0e965e5bba02be2946e85656c513a3e44e0d5f5` deployed successfully. Live browser and HTTP checks confirmed the three-step install page, removed invitation paragraph, alpha.6 download, updated connection/changelog/agent guides, and the signed feed matching source byte-for-byte with builds 7, 6, 5, 4, and 3. Evidence: `.local/release-check-alpha6/publication.json`, checked at `2026-09-07T15:35:28Z`.

The subsequent publication CI run `34139046051` exposed an intermittent failure in the existing process-lock regression; all four new first-run tests passed. A local POSIX reproduction confirmed that even a close-on-exec descriptor can briefly keep a closed parent's lock alive while an unrelated child is between fork and exec. This is consistent with parallel tests launching subprocesses, though the exact child involved in that CI run was not identified. The regression now allows a bounded wait for that transient condition while keeping its controlled child alive throughout. In an isolated checkout with close-on-exec deliberately removed, the revised regression still failed as required, proving that it continues to detect a real inherited lock. Only the test changed; the signed installer is unchanged. Evidence: `.local/release-check-alpha6/pre-exec-lock-window.json` and `store-lock-mutation.log`.

The revised test passed five consecutive runs of the full 27-test parallel Swift suite. The loopback integration and six collector checks also passed. Evidence: `.local/release-check-alpha6/store-lock-parallel-1.log` through `store-lock-parallel-5.log`.


## Automatic opening at login — September 7, 2026 PDT

Implemented default Open at login for the first normal launch of a release app from Applications, including upgrades from versions without this feature. The macOS registration state is authoritative; a version-independent preference prevents repeated registration and preserves later app/System Settings choices. Paused participation remains unchanged. Disk-image, development/demo, and isolated custom-profile sessions do not register or consume the default. Your contribution contains the control and guidance for a system approval requirement.

All 33 Swift tests and the loopback integration suite passed, along with six collector tests and six release/update tests. Six startup-policy tests cover one-time registration, saved opt-out, external system changes, registration/removal failure, approval-required results, initial not-found status, and excluded sessions. The website build passed.

A temporary Developer ID signed app in this user's Applications folder compiled the actual production startup factory, controller, and SwiftUI control with its own bundle identifier and preference domain. Real `SMAppService` registration reached enabled status automatically from a fresh preference. The checkbox enabled and unregistered it, and the disabled choice remained after a verified new process launch. Native accessibility and a screenshot confirmed the control and explanation. The check initially exposed a not-found state before first registration; attempting registration once from that eligible state fixed the default and is covered by a regression test.

The temporary service was explicitly unregistered, its state confirmed not registered, and its app/preferences removed. The installed pilot app and node data were unchanged. This verifies registration and persistence through the real macOS service, not a completed logout/login or reboot. That field check remains in the host guide. Evidence: `.local/startup-check-alpha7/verification.json` and its state snapshots and test log. Apple API behavior and implementation scope are documented in [startup behavior](STARTUP.md).


### Signed automatic-startup release

Built universal `0.2.0-alpha.7` (build 8) from clean commit `e69b5225096a3253204a53c503786c8faba58877`. All three GitHub Checks jobs passed for that exact source ([run 34140409623](https://github.com/toddsherman/fourthciv/actions/runs/34140409623)). Apple accepted app submission `8bb07d40-4a50-4059-8629-c5619e842a0e` and DMG submission `ce908be2-17a2-49b0-94b8-fc787dcf948e` without issues. The app and installer were stapled and passed distribution/Gatekeeper checks. Both app and CLI contain arm64 and x86_64 executables; the bundled CLI passed notarization verification.

Mounted the final DMG read-only, copied and verified its unmodified app, and ran it against an isolated saved profile. It reported alpha.7/build 8 and retained paused participation, enabled internet configuration, 8 MiB storage, and 5 MiB daily sync limits. Its isolated launch left the normal app's login preference unchanged. The UI automation tool subsequently reopened that copy without the test environment; the app refused the already occupied default store. A separate UI copy with a unique bundle identifier and Launch Services test environment then verified the complete Your contribution panel, including the disabled Open at login control outside Applications and the preserved paused state/limits. The distributed DMG was unchanged. Test processes were stopped and no public events were posted. Evidence: `.local/release-check-alpha7/verification.json` and `ui-check.json`.

The install page passed local visual, DOM, and console checks: it retains three steps, explains automatic startup and its control, and points at alpha.7. Published the immutable DMG, checksum, and manifest as the GitHub prerelease. Its public download passed SHA-256 (`b363798474dc3b11e89d568d340c7d5b05bee1794028f6d79cf67340f460524a`) and Ed25519 verification before staging the signed update feed. Existing builds 7 through 3 are preserved beneath build 8. The following website publication carries the matching instructions and exact signed feed. Actual restart/login testing on a physical host remains pending.


## September 11 — Host welcome, connection status, and bounded sender

Release `0.2.0-alpha.9` (build 10), source `22f815de827d255287bb00167287f2060db83335`, passed 41 Swift tests, loopback integration, 16 collector/sender tests, six update tests, and the native app build. All three source CI jobs passed ([run 34673746508](https://github.com/toddsherman/fourthciv/actions/runs/34673746508)). The eight new host-status tests cover initial receipt, failed/partial exchanges, pause and configuration changes, pending pagination, resource limits, and historical success. The UI uses observed sync results while retaining the existing CLI and diagnostics health values.

An isolated native preview showed the welcome, optional agent connection, and contribution controls. A real failed DNS request changed the display from waiting to retrying with a countdown; pausing replaced it with the paused explanation. The preview used its own profile and was stopped afterward. The exact signed universal installer passed Gatekeeper and CLI notarization checks, then launched with an isolated profile and preserved pause, custom storage/data limits, and the existing login-item preference. The published download passed SHA-256 and Ed25519 verification. Evidence is in `.local/release-check-alpha9/`.

The finite sender uses sixteen slots over eight hours, reserves each slot before one post attempt, skips missed slots, and never retries an uncertain submission. Offline checks cover a complete virtual run, stop requests, locks, private files, identity mismatch, duplicate visibility, and reconciliation. These checks do not establish overnight delivery. Collectors for `OVERNIGHT-20260911-A` were started before the sender: Mac A was directly inspected, while Mac C and Mac D readiness was confirmed by the user. Actual receiver versions, receipt IDs, and overnight outcomes must be taken from collected evidence.


## September 12 — Completed active overnight test and receiver updates

Run `OVERNIGHT-20260911-A` completed eight hours automatically, from September 11 21:47:35 to September 12 05:47:36 PDT. All 16 slots were acknowledged with no skipped or uncertain submissions. Direct installed-CLI reads verified every exact signed event once on Mac A and the relay, with identical envelopes; all 23 prior events remained unchanged. Mac A contributed 480 active-window samples with 904 successful relay syncs, no recorded failures or CLI errors, no sampling gaps, and one alpha.9/build 10 process/session.

The host supplied Markdown assessment reports from Mac C and Mac D. All 32 receiver table rows match the sender's exact IDs, report one occurrence, and give first observations within the sender window. Both reports describe fresh successful signature-verifying CLI reads and no later catch-up requirement. This supports the bounded three-Mac delivery test; raw receiver logs/fresh-read JSON were referenced by those reports but not independently copied here. C and D ran alpha.8/build 9 during collection. C reported 14 sampling gaps over 90 seconds (seven wholly during sending, one across its end, six afterward); their causes are unknown. D had no recorded failures during sending, but fresh diagnostics showed a subsequent network failure followed by success 32 seconds later. Memory and CPU observations remained modest for this workload; D had a small RSS increase, so this is not a claim of zero memory growth or long-term leak freedom.

The sender stopped automatically. All collectors were subsequently stopped (A directly verified, C/D confirmed in supplied reports), and the 8 a.m. review automation was paused. The combined report, original supplied reports, SHA-256 digests, and programmatic ID comparison are preserved under the local run's `assessment-20260912` folder. The host subsequently confirmed C and D updated to alpha.9; this is a version confirmation, not a fresh exact-build, history/settings, or receiver-UI comparison. It does not change the versions that participated overnight.

## September 12 — Resource caps and recovery on alpha.9

Three new focused production-core lifecycle tests passed in 32.372 seconds. Valid signed fixtures fill the configured 1 MiB store; a rejected next event does not alter saved history, and the cap/history survive reopening. Raising capacity permits completion at the actual pending retry deadline. A separate fixture consumes the configured 1 MiB daily allowance through transfer accounting, verifies zero further transport calls even after pause/resume and reopening, then recovers after an increase. A deterministic UTC-boundary check preserves new-day usage and relay progress when an old reservation settles late. The transport is injected and local; these are not public-TLS bandwidth-exhaustion tests.

The installed alpha.9 bundled CLI also passed a real loopback-HTTP storage check with internet/LAN sharing disabled. A 1 MiB node retained 63 verified events totaling 1,041,586 bytes, rejected the next valid signed event, retained the exact history and cap after process restart, then accepted that same rejected ID after the limit was raised to 8 MiB. The resulting 1,058,375-byte store proves the original cap was crossed only after the increase; replay returned already-present without duplication.

A separate copy of the installed app used a unique bundle ID/custom data directory and was ad-hoc signed without changing release code. Its ledger was precharged to the UI's 5 MiB allowance, with copies of 39 already-public signed events and acknowledged relay progress. Native accessibility checks showed Daily sync limit reached and the UTC-reset explanation, blocked manual sync, preserved the cap and readable history after relaunch, then returned to Conversations synced after the UI allowance increased to 25 MiB. The actual HTTPS exchange reported received 0/shared 0; the increased allowance persisted. This seeded boundary-state check complements, rather than substitutes for, the transfer-consumption test.

All temporary node/app processes stopped. The live app's settings and signed event IDs, and the public relay's event ID set, were unchanged. No new public fixture messages were published. No production defect or app-code change was needed. Evidence and the local summary are in `.local/resource-limits-alpha9/`. The new test file remains suitable for repeatable local/CI checks.

## September 12 — First-visit guidance and conversation navigation

The native reader now keeps its hosting explanation until dismissed, independently of whether conversations have arrived. Its choice is saved per local profile, and About hosting brings the guide back into view even from a scrolled feed. A reader visit tracks newly observed local message IDs separately from author-supplied dates; it does not infer live authors, unread status, or publication recency. Saved messages remain available while paused or disconnected.

Six conversation-index tests cover parent-before-child order with backdated replies, deterministic ties, complete lookup despite search filtering, distinct signing identities sharing/changing names, defensive incomplete/cyclic references, and 2,000-message deep/wide conversations. All 50 Swift tests passed, along with real loopback integration checks and the six collector/ten sender tests.

Isolated native previews verified the empty guide, receipt labeling, guide dismissal across app relaunch, the guide reopening from a scrolled feed, search-to-full-conversation navigation, returning to the original search, parent jumps with highlighting, and different signing identities with the same displayed name. A separate profile containing only copies of the 39 already-public events completed a real HTTPS sync (received 0/shared 0), showed the quiet saved-history explanation, and preserved readable history after pausing through Your contribution. Default and minimum-width windows were visually checked; a truncated optional-connection label was shortened while retaining optional guidance. Test fixtures stayed in the offline profile. Release-package and publication checks are recorded separately below when complete.


## September 12 — Alpha.10 release package

Release `0.2.0-alpha.10` (build 11) was built from clean commit `446e83283089f2a492abbdf23d4db897efb117cf`. All three GitHub Checks jobs passed for that exact source, including protocol/integration/interoperability, relay, landing-page, and release metadata checks. The universal app and DMG were Developer ID signed, notarized, and stapled. The exact packaged app and bundled CLI passed signature, architecture, Gatekeeper/distribution, and CLI notarization checks. An isolated launch preserved all 39 copied signed events, pause, custom 8 MiB storage / 5 MiB daily limits, and the existing login-item preference. It was stopped afterward without replacing the normal pilot app or publishing messages.

The immutable installer was published as a GitHub prerelease. SHA-256 and Ed25519 verification of the actual public download passed before staging the signed update feed; previously published feed entries were retained. Package evidence is in `.local/release-check-alpha10/`; native preview observations are in `.local/reader-first-visit/assessment.md`. Physical-Mac installation results remain separate from these isolated checks.

The production feed was verified byte-for-byte against the signed local feed; the live install page and changelog referenced alpha.10. This Mac then updated from alpha.9/build 10 through the normal App updates → Install Update → Install and Relaunch flow with the update panel open. Fresh installed-CLI reads confirmed alpha.10/build 11, the same 39 exact signed events, and unchanged contribution settings. The installed app passed Gatekeeper again. Native inspection showed the new hosting guide, saved-history explanation, full dates, and conversation links after relaunch. Evidence is in `.local/release-check-alpha10/installed-update.json` and `installed-diagnostics.json`. C/D updates are not claimed by this check.


## September 12 — Alpha.11 menu redesign package

Release `0.2.0-alpha.11` (build 12) was built from clean commit `a99542b079ae94d70d9c97d5eacc7ac2775ab2d9`. All 50 Swift tests, loopback integration, 16 pilot tests, six release-safety tests, and the updater callback regression passed. All three source CI jobs passed ([run 34707322305](https://github.com/toddsherman/fourthciv/actions/runs/34707322305)). The native menu view was visually checked in an isolated window harness for synced, paused, and storage-warning states; Pause/Resume and the secondary menu were exercised. Separate preview hosts checked the real signed feed for current and available versions.

The universal app and DMG were Developer ID signed, notarized, and stapled. Apple accepted app submission `c888de9d-d9fd-49fc-bbc0-580aff77855a` and DMG submission `b6fff5e4-7c54-40da-80b4-c39586c4302e` without issues. The exact packaged app and CLI passed signature, architecture, Gatekeeper/distribution, and CLI notarization checks. An isolated launch with updater checks disabled preserved all 39 copied verified public events, pause, 8 MiB storage / 5 MiB daily limits, and the existing login-item preference; its process was stopped afterward.

The immutable installer was published as a GitHub prerelease. Its SHA-256 is `42ec8be37c7127164be3323e2356c44b701a13bc942ac7aa81915b76cc0f34a2`. Package and CI evidence is in `.local/release-check-alpha11/`; the isolated exact-package run is in `.local/release-check-alpha11-t6c04g7d/`. Public feed and installed-update verification follow separately.

The public installer passed SHA-256 and Ed25519 checks before the exact signed feed was deployed. Vercel production deployment `dpl_FMwsTrU5n6aZVRQSSvwaRhdaV27Y` served main commit `12985346231b0da0aecaf37427f7c0d565ae2f6e`; the live feed matched byte-for-byte, verified with the app public key, and the install page/changelog referenced alpha.11. This Mac then completed the normal Sparkle update from alpha.10/build 11 to alpha.11/build 12. Fresh installed-CLI reads preserved all 39 exact signed events and unchanged contribution settings and login preference. The installed app and CLI match the verified package binaries and passed Gatekeeper/signature checks. Native inspection confirmed build 12 and automatic update checks still enabled. Evidence: `.local/release-check-alpha11/publication.json`, `installed-update.json`, and `installed-diagnostics.json`. Other physical Macs were not updated or verified by this check.
## Pre-friend hardening candidate — September 12, 2026

Prepared on `codex/pre-friend-hardening` from alpha.10 baseline `35dda6c5e521bcf74f9940414f3bb55b399097ab` in an isolated worktree. After the parallel menu/updater work published alpha.11, the candidate was rebased onto its recorded release commit `dd67901`, retaining the latest menu and updater changes. The test results below initially verified the hardening code before that rebase. This is a local candidate, not a new public installer or deployed relay. GitHub private vulnerability reporting was enabled and read back as enabled; no other hosted setting, production schema/history, installed app, or signed update feed changed.

The relay now admits requests against persistent per-source-network allowances before charging the shared admission allowance: 120/minute and 60,000/day per IPv4 address or IPv6 /64, with 1,200/minute and 120,000/day across the relay. IPv4-mapped addresses normalize to IPv4. A dedicated server key produces rotating daily HMAC identifiers; application storage contains pseudonymous counters rather than raw addresses. Missing trusted source metadata or secret fails closed. Rejected source requests leave shared counters unspent. Existing public-event storage, publication rules, and epoch are preserved by an additive schema migration. This does not prevent distributed denial of service or cap hosting costs; denied requests still use infrastructure. See the security review for residual spam/capacity risk and shared-NAT limitations.

The native client now schedules successful idle polls after 60 seconds; new received/shared events or pending work retain 30 seconds. Failure backoff remains 30–300 seconds. Messages arriving during an idle interval can wait until that deadline plus normal timer delay. A deterministic scheduling clock is internal to tests; production uses real time.

Verified on this Apple silicon build Mac:

- `bash scripts/test.sh`: all **63 Swift tests in 13 suites** passed, including real loopback redirect/body-size/truncation/cancellation/timeout cases, actual atomic-write failures, malformed persisted snapshots, lock cleanup, and deterministic polling/retry checks. The Swift portion took 33.0 seconds. All real-process loopback integration checks, six collector tests, and ten sender tests also passed.
- `npm test --prefix relay`: the original five tests and eight initial request-admission tests passed. The migration case was then split and strengthened with the exact frozen alpha.10 schema, giving **14 total relay tests**; both changed cases passed separately. The migration test upgrades old tables, reapplies the new schema, and preserves synthetic stored history, epoch, and old rate counters.
- The full-day admission test exercised all 46,080 discovery/page requests for eight continuously active hosts, with another source's full 60,000-request daily allowance already charged: 106,080 admitted requests stayed within the 120,000 global ceiling. Committing simulated hours avoids one unrealistic day-long database transaction; the final test passed in 10.8 seconds. This is a PGlite admission exercise, not a live throughput benchmark or independent Neon multi-connection concurrency test.
- `npm run test:interop --prefix relay`: passed the real Swift-to-JavaScript/PostgreSQL signed Unicode community/message/reply exchange and tamper rejection.
- `npm run test:outage --prefix relay`: passed with production timers in 134.9 seconds. Readable history and a durable queued post survived HTTP 503; automatic recovery after service restoration took 90.3 seconds, with the exact queued event present once in both nodes and the relay. Temporary test processes, server, database and identities were cleaned up. Evidence: `.local/relay-outage-check/result.json` within the candidate worktree.
- `python3 scripts/test_updates.py`: all six signature/release tests passed. No update feed or installer was regenerated.
- `bash scripts/build-app.sh` and `codesign --verify --deep --strict 'dist/Fourth Civ.app'`: passed for the local ad-hoc debug app. This is not notarization or a distributable release verification.
- `npm run build --prefix website`: passed. Production dependency audits for both relay and website reported zero known vulnerabilities.
- All eight secret-scanner tests passed. A redacted current-source and reachable-history pattern scan reported no suspected secrets or scan errors; counts and exclusions are recorded in `docs/SECURITY_REVIEW.md`.

Regular CI now runs secret scanning, high/critical production-dependency audit gates, and the existing native relay-outage test. Release CI also gates packaging on the scan, relay audit, and outage recovery. These workflow edits were validated locally; no remote CI run is claimed for this unpublished branch.

Independent source review found no blocking regression in the candidate. Before promoting it, complete `docs/RELAY_HARDENING_ROLLOUT.md`: configure the separate preview database and secret, verify actual platform-header replacement and independent PostgreSQL transaction concurrency, then perform the ordered production migration/deployment and read-back checks. A new signed/notarized Mac release and the guided fresh-Mac installation checklist remain separate gates. Alpha.11 remains the published download.


After rebasing onto the published alpha.11 record (`dd67901`), the complete native wrapper passed again: 63 Swift tests in 32.6 seconds, all loopback integration checks, six collector and ten sender tests. The combined local app rebuilt and passed strict deep signature verification. All six release-signature tests passed against the alpha.11 feed, and the retained alpha.11 updater-status harness passed its checking/failure/current/incompatible/available/dismissed/skipped/canceled cases. The harness emitted its existing Sparkle `SUAppcastItem(dictionary:)` deprecation warning; the app build succeeded. The alpha.11 website build and all 25 workflow shell-step syntax checks passed. The rebase retained the existing updater-status CI gate and public alpha.11 metadata/feed. The candidate's relay code remained separate from the public service.
