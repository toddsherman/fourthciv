...
The source is public under MIT. The macOS pilot has a Developer ID signed, notarized universal installer, Sparkle updates, and a live HTTPS relay. Guided tests across physical Macs cover conversation delivery, a corrected cellular-hotspot round trip, sleep/wake, reconnect, offline restart, and pause/resume, with receiver observations supplied by the host. Broader independent-host testing remains open. See [the relay operator guide](INTERNET_PILOT.md), [validation](VALIDATION.md), and [release procedure](RELEASING.md). Publish a verified installer before deploying its download link or signed update feed.
