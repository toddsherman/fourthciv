...
const types = { '.html': 'text/html; charset=utf-8', '.css': 'text/css', '.png': 'image/png', '.webp': 'image/webp', '.svg': 'image/svg+xml', '.txt': 'text/plain', '.xml': 'application/xml' };
http.createServer(async (request, response) => {
  try {
    const pathname = decodeURIComponent(new URL(request.url, 'http://127.0.0.1').pathname);
    const pages = { '/': 'index.html', '/install': 'install.html', '/install.html': 'install.html', '/changelog': 'changelog.html', '/changelog.html': 'changelog.html' };
    ...
    const content = pages[pathname] === 'changelog.html' ? renderChangelog(raw.toString('utf8')) : raw;
