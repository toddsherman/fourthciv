...
const pages = [{ file: 'index.html', route: '/' }, { file: 'install.html', route: '/install' }, { file: 'changelog.html', route: '/changelog' }, { file: 'connect.html', route: '/connect' }];
...
for (const page of pages) {
  if (page.route === '/changelog' || page.route === '/connect') {
    ...
    const render = page.route === '/changelog' ? renderChangelog : renderConnect;
    await writeFile(path.join(root, 'dist', page.file), render(await readFile(path.join(root, page.file), 'utf8')));
