...
**[Download Fourth Civ for Mac](https://github.com/toddsherman/fourthciv/releases/download/v0.2.0-alpha.13/FourthCiv-0.2.0-alpha.13.dmg)** · [Release notes and checksums](https://github.com/toddsherman/fourthciv/releases/tag/v0.2.0-alpha.13)

1. Open the downloaded DMG and follow the two steps in the installer. Drag **Fourth Civ** into **Applications**. Quit an older copy first and choose **Replace** if prompted.
2. Open the installed Fourth Civ copy from Applications, then eject the installer disk. Look for **IV** in your menu bar. Starting with alpha.7, the app also enables **Open at login** on its first normal launch from Applications; you can turn it off in Your contribution or macOS Login Items.
...
This is **alpha.13 (build 14)**, an early pilot prerelease. The installer now shows the drag-to-Applications and open steps directly, with Installation help for text and keyboard instructions. Follow the [guided friend-install checklist](docs/PILOT_HOST_GUIDE.md#guided-friend-checklist) and see the [verification record](docs/VALIDATION.md). Intel execution, older supported macOS versions, and a friend's fresh installation still need field verification.
...
Alpha.12 combines the pixel IV menu icon and slower idle polling with real transport and filesystem-failure regressions. The relay has separate source-network request allowances; see the [security review](docs/SECURITY_REVIEW.md), [rollout record](docs/RELAY_HARDENING_ROLLOUT.md), and [validation](docs/VALIDATION.md) for deployment evidence and remaining pilot checks. The secret scanner checks tracked working files without printing matched content; use `--include-untracked --history` to additionally check non-ignored additions and locally reachable Git history.
